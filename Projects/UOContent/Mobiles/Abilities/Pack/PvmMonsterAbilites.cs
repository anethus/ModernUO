...
namespace Server.Custom.PvM
{
    ...
    public partial class PvmMonsterAbilities
    {
        ...
        //                    new FireFieldItem(14232, pnt, from, from.Map, TimeSpan.FromSeconds(Utility.Random(6, 12)), 10, 30, 96);
        //                }
        //            });
        //        }
        //    });
        ...
        //                {
        //                    var point = points[i];
        //                    IPooledEnumerable<Mobile> eable = null;
        //                    try
        //                    {
        //                        using var pool = PooledRefQueue<Mobile>.Create();
        //                        eable = from.Map.GetMobilesInRange(point, 1);
        //                        foreach (var target in eable)
        //                        {
        //                            if (!WitherSpell.ValidTarget(from, target)
        //                             || (expiryHits.TryGetValue(target, out var expiry) && Core.TickCount >= expiry))
        //                                continue;
